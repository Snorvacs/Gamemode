GM:AddMenuPanel( { name = "Jobs" }, function( self, parent )
	local Open = 1
	...
	function button:Paint( w, h )
		draw.SimpleText( "Job", "cgMedFont", w / 70, h / 2, Color( 235, 235, 235 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
	...
	for i, skill in ipairs( self.JobSkills ) do
		local panel = vgui.Create( "DPanel", parent )
		panel:Dock( TOP )
		...
		function button:Paint( w, h )
			...
			draw.SimpleText( tostring( LocalPlayer():GetJobLevel( i ) ), "cgSmallFont", w / 7, h / 2, Color( 235, 235, 235 ), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )

			if LocalPlayer():GetJobLevel( i ) == 99 then
			...
			if !PrevVal then
				PrevVal = map( LocalPlayer():GetJobExp( i ), 0, GAMEMODE.LevelValues[ LocalPlayer():GetJobLevel( i ) + 1 ], 0, w * 0.83 )
			end

			local Val = math.Clamp( PrevVal + 8, 0, map( LocalPlayer():GetJobExp( i ), 0, GAMEMODE.LevelValues[ LocalPlayer():GetJobLevel( i ) + 1 ], 0, w * 0.83 ) )
			PrevVal = Val

			surface.SetDrawColor( 200, 200, 200, 10 )
			surface.DrawRect( w / 6.5, h / 2 - h * 0.3, Val, h * 0.6 )

			draw.SimpleText( tostring( LocalPlayer():GetJobExp( i ) ) .. "/" .. tostring( GAMEMODE.LevelValues[ LocalPlayer():GetJobLevel( i ) + 1 ] ), "cgSmallFont", w / 6.5 + w * 0.83 / 2, h / 2, Color( 235, 235, 235 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
		...
		GuidePanel:SetHTML( skill:GetGuide() )
